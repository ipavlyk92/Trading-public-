...
# Створюємо об'єкт часового поясу UTC+2
...
# Візуалізація таймера та поточного часу UTC+2 у боковій панелі
st.sidebar.markdown(f"🕒 Поточний час (UTC+2): **{get_now().strftime('%H:%M:%S')}**")
st.sidebar.markdown(f"### ⏳ Оновлення через: `{int(remaining)//60:02d}:{int(remaining)%60:02d}`")
...
@st.cache_data(ttl=300) # Кеш також на 5 хвилин
...
if not data.empty:
    ...
    # 2. Графік Z-Score з детальною шкалою
    ...
    # Детальна сітка (Major - 2 години, Minor - 30 хв)
    ax2.minorticks_on()
    ...
    with col1: st.metric("Ціна", f"${last['Close']:.2f}")
    with col2: st.metric("Z-Score", f"{last['Z_Score']:.2f}")
    with col3:
        if last['Z_Score'] > 2.5: st.error("🔔 ПРОДАЖ")
        elif last['Z_Score'] < -2.5: st.success("🔔 КУПІВЛЯ")
        else: st.info("⚖️ Рівновага")

else:
    st.error("Дані відсутні. Спробуйте інший період.")
